...
if (isPost()) {
  ...
  //Validate tên danh mục blog: Bắt buộc nhập, >=4 ký tự
  if (empty(trim($body['name']))) {
    $errors['name']['required'] = 'Tên danh mục blog bắt buộc phải nhập';
  } else {
    if (strlen(trim($body['name'])) < 4) {
      $errors['name']['min'] = 'Tên danh mục blog phải >=4 ký tự';
    }
  }
  ...
  if (empty($errors)) {
    ...
    if ($insertStatus) {
      ...
      redirect('admin?module=blog_categories'); //Load lại trang thêm danh mục blog người dùng
    }
  } else {
    ...
    redirect('admin?module=blog_categories'); //Load lại trang danh mục blog người dùng
  }
}
