...
$allPagesNum = getRows("SELECT id FROM `blog_categories` $filter");
...
$listCate = getRaw("SELECT *, (SELECT count(blog.id) FROM blog WHERE blog.category_id = blog_categories.id) as blogs_count FROM `blog_categories` $filter ORDER BY create_at DESC LIMIT $offset, $perPage");
          ...
          <tbody>
            ...
            ?>
                <tr>
                  ...
                  <td><a href="<?php echo getLinkAdmin('blog_categories', '', ['id' => $item['id'], 'view' => 'edit']); ?>"><?php echo $item['name']; ?></a><?php echo ' (' . $item['blogs_count'] . ') '; ?><a href="<?php echo getLinkAdmin('blog_categories', 'duplicate', ['id' => $item['id']]); ?>" style="padding: 0 5px;" class="btn btn-danger btn-sm">Nhân bản</a> <a href="#" style="padding: 0 5px;" class="btn btn-success btn-sm" target="_blank">Xem</a></td>
                  ...
                  <td class="text-center"><a href="<?php echo getLinkAdmin('blog_categories', 'delete', ['id' => $item['id']]); ?>" id="delete_sweet_alert2" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></a></td>
                </tr>
              <?php
              endforeach;
            else :
